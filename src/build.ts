...
import { justDot, justDotGrayscaleFolder, justDotRoot } from "./themeJustDot";
import {
  justDotSmall,
  justDotSmallGrayscaleFolder,
  justDotSmallRoot,
} from "./themeJustDotSmall";
...
async function generate(iconTheme: IconTheme): Promise<VSCodeIconTheme> {
  ...
  for (const icon of iconTheme.icons) {
    await addIcon(
      result,
      icon,
      Object.assign({}, iconTheme.default, icon.dark)
    );
    if (icon.light) {
      if (!result.light) {
        result.light = {};
      ...
      await addIcon(
        result,
        icon,
        Object.assign({}, iconTheme.default, icon.light)
      );
    }
  }
...
function addFolderIconConfig(
  ...
  if (icon.folderNames) {
    iconConfig.folderNames = Object.assign(
      {},
      iconConfig.folderNames,
      fromArray(icon.folderNames, key)
    );
    iconConfig.folderNamesExpanded = Object.assign(
      {},
      iconConfig.folderNamesExpanded,
      fromArray(icon.folderNames, expandedKey)
    );
  }
}

function addFileIconConfig(
  iconConfig: IconConfig,
  key: string,
  icon: FileIcon
) {
  ...
  if (icon.fileExtensions) {
    iconConfig.fileExtensions = Object.assign(
      {},
      iconConfig.fileExtensions,
      fromArray(icon.fileExtensions, key)
    );
  }
  if (icon.fileNames) {
    iconConfig.fileNames = Object.assign(
      {},
      iconConfig.fileNames,
      fromArray(icon.fileNames, key)
    );
  }
  if (icon.languageIds) {
    iconConfig.languageIds = Object.assign(
      {},
      iconConfig.languageIds,
      fromArray(icon.languageIds, key)
    );
  }
}
...
async function createSvgKey(
  ...
  if (!iconDefinitions[key]) {
    ...
    let targetSvg = svg
      ...
      .replace(
        /"#CCC"/gu,
        `"${color?.color || color?.background2 || color?.background}"`
      );

    if (color.grayscale && color.grayscale > 0) {
      targetSvg = targetSvg.replace(
        /fill=/gu,
        `filter="grayscale(${color.grayscale}%)" fill=`
      );
    }
